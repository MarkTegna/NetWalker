"""
Query SEP Device Information

This script queries the database to find information about Cisco IP Phone (SEP) devices.
"""

import pyodbc
from netwalker.config.config_manager import ConfigurationManager

def query_sep_devices():
    """Query database for SEP device information"""
    ...
    try:
        ...
        # Query for SEP devices (Cisco IP Phones)
        cursor.execute("""
            SELECT 
                d.device_id,
                d.device_name,
        ...
        if not rows:
            print("No SEP (Cisco IP Phone) devices found in database")
        else:
            print(f"Found {len(rows)} SEP device records:\n")
        ...
        if not neighbor_rows:
            print("No neighbor connections found for SEP devices")
        else:
            ...
            for row in neighbor_rows:
                ...
                print(f"SEP Device: {row[2]}")
                print(f"SEP Interface: {row[3]}")
