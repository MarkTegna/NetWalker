"""
Query s2node Device Information

This script queries the database to find information about the s2node device.
"""
...
def query_s2node():
    """Query database for s2node device information"""
    ...
    try:
        ...
        cursor.execute("""
            ...
            WHERE d.device_name LIKE '%s2node%'
        ...
        if not rows:
            print("No s2node devices found in database")
        else:
            print(f"Found {len(rows)} s2node device records:\n")
        ...
        cursor.execute("""
            SELECT 
                ...
                dn.source_interface,
                dd.device_name as s2node_device,
            ...
            WHERE dd.device_name LIKE '%s2node%'
        ...
        if not neighbor_rows:
            print("No neighbor connections found for s2node devices")
        else:
            ...
            for row in neighbor_rows:
                ...
                print(f"s2node Device: {row[2]}")
                print(f"s2node Interface: {row[3]}")
