"""Purge all Polycom* devices from the database."""
import pyodbc
import base64
...
cursor.execute("SELECT device_id, device_name FROM devices WHERE platform LIKE '%Logitech%'")
devices = cursor.fetchall()
print(f"Found {len(devices)} Polycom devices to purge")
