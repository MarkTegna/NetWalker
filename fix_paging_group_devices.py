"""
Fix Paging Group Devices in Database

This script updates existing paging group devices in the database to have the correct
...
def fix_paging_group_devices():
    """Update paging group devices in database"""
    ...
    try:
        ...
        print(f"Found {len(devices)} paging group devices to update:\n")
        ...
        for device in devices:
            ...
            # Update platform
            new_platform = "Cisco Paging Group"
            
            # Update hardware model if it's generic
            new_model = current_model
            if current_model in ['Unwalked Neighbor', 'unknown', None]:
                new_model = "Cisco Unified Communications Paging Group"
            
            # Add VoIP capability if not present
            capabilities_list = []
            if current_capabilities:
                capabilities_list = [c.strip() for c in current_capabilities.split(',')]
            
            if 'VoIP' not in capabilities_list and 'voip' not in [c.lower() for c in capabilities_list]:
                capabilities_list.append('VoIP')
            
            new_capabilities = ','.join(capabilities_list) if capabilities_list else 'VoIP'
        ...
        print(f"\n✓ Successfully updated {updated_count} paging group devices")
